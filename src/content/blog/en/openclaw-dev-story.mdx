---
title: 'From Clawdbot to Moltbot to OpenClaw 🤦‍♂️'
...
# From Clawdbot to Moltbot to OpenClaw 🤦‍♂️
...
OpenClaw (formerly Moltbot, formerly Clawdbot) ㅋㅋㅋㅋㅋㅋㅋㅋ

It's been a while since I've seen an OSS project do a full identity crisis in public like this, and honestly it's kind of entertaining ㅋㅋㅋ 
...
- It's a self‑hosted personal AI assistant that you run on your own machine, VPS, or even Cloudflare Workers.
- You talk to it through channels you already use: WhatsApp, Telegram, web dashboard, terminal TUI, etc.
- Under the hood it can talk to whatever LLM you prefer – Claude, OpenAI, Gemini, or a local model via Ollama.
...
> "An AI assistant that lives on your infra and actually does stuff – runs commands, hits APIs, sends emails, manages your calendar – all from a chat app."
...
the fix was obvious: set `ANTHROPICAPIKEY` / `OPENAIAPIKEY` env vars properly and restart.

### 3) Second gotcha: losing the gateway token

Some hosting guides generate an `OPENCLAWGATEWAYTOKEN` and tell you to store it somewhere safe because you'll need it to access the web interface. 
Of course I did not store it anywhere.

Later, when I opened the web UI, it politely asked me to enter the gateway token. 
Cue the "oh… it was that token" realization.
...
Skills can send emails, schedule calendar events, manipulate git, touch your filesystem – all triggered from chat.
...
If you're coming in fresh, you end up cross-referencing 2–3 guides just to figure out which name maps to which phase.
...
## Scenario: Telegram → Notion calendar → email blast
...
This is the "agent" part in a nutshell: chat → LLM → structured intent → downstream APIs.
...
const notion = new Client({
 auth: process.env.NOTIONAPIKEY, // store in env or secret manager
});

const NOTIONDATABASEID = process.env.NOTIONEVENTDB_ID!;
...
export async function createNotionEvent(event: ParsedEvent) {
 ...
 parent: { databaseid: NOTIONDATABASE_ID },
 properties: {
 Name: {
...
That's the classic "why isn't this working?" → "oh, wrong property type" moment.
...
const EMAILFROM = process.env.MAILFROM!;
const EMAILTOLIST = (process.env.MAILTOLIST ?? "")
 .split(",")
 .map((x) => x.trim())
 .filter(Boolean);
...
1. OpenClaw is a self‑hosted agent, not just a UI 
...
4. Telegram → Notion → email is just channels + LLM parsing + two APIs 
