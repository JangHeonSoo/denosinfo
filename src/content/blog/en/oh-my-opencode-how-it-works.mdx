...
The project describes itself as "the best agent harness" and a "batteries-included" plugin for OpenCode. Architecturally, it keeps OpenCode as the core engine and wraps it with:

- A primary orchestrator agent (Sisyphus).
- Multiple specialized sub-agents (Oracle, Librarian, Explore, Frontend, etc.).
- A rich tool layer (LSP, AST-Grep, file system, Git, session search).
- A compatibility layer that reads Claude Code config and maps it into OpenCode's format.
...
- Decomposes tasks: It takes your prompt (often containing keywords like `ultrawork`) and splits it into design, research, implementation, and testing steps.
- Delegates to sub-agents: Architecture to Oracle, code search to Explore, docs/codebase exploration to Librarian, and so on.
- Retries and pivots: If one approach fails, it tries alternative tools or strategies. In demo videos, the narrator explicitly mentions that it "recognizes when something is not working and pivots".
- Enforces continuation: With a todo-enforcement mechanism, it avoids stopping halfway through. If some steps remain, it nudges itself to continue until they are done.
...
- Oracle – architecture analysis, code review, and refactoring strategies.
- Librarian – documentation and codebase exploration, understanding legacy code.
- Explore – fast code search and symbol tracing.
- Frontend / Backend agents – focused implementation for specific layers.
- Background agents – tasks that run with `runinbackground: true`, allowing parallel work.

The `callomoagent` tool lets one agent spawn another, with an optional `runinbackground` flag so the caller does not block on the result. In practice, this means multiple agents can work in parallel on different parts of the same project, much like a real engineering team.
...
- LSP-based features: go-to-definition, find references, symbol search, diagnostics, and code actions (like rename and quick fixes).
- AST-Grep: AST-aware code search and replace for 25+ languages; matches patterns at the syntax tree level instead of raw text.
- Session history search: reuse previous changes or discussions without reloading them into context.
...
> "Why should IDE tools be for humans only, and not for agents?"
...
- Read PR discussions, issues, or Slack threads to understand project context.
...
If you do not want to micromanage agents and tools, there is a shortcut: include `ultrawork` (or the shorter `ulw`) in your prompt.

Blog reviews explain that when `ultrawork` is present, Sisyphus shifts into full-orchestration mode:

- It automatically breaks the request into stages.
- It spins up relevant sub-agents in parallel.
- It leverages LSP/AST tools when editing code.
- It drives the loop from "initial draft" through "refinement" to "cleanup."
...
- A single request triggers multiple agents working on different layers.
- The system does not quit after a single failed attempt; it tries alternative paths.
- Edits feel more like IDE-backed refactors than simple text generation.
...
- Orchestration – Sisyphus splits work and coordinates multiple sub-agents, often in parallel.
- Tool integration – agents can invoke LSP, AST-Grep, MCP, file and Git tools directly.
- Compatibility and productivity – Claude Code configs are reused, and keywords like `ultrawork` flip on the full stack in one go.
