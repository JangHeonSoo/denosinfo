...
Matt Schlicht라는 개발자가 만든 OpenClaw. 레딧이랑 똑같이 생겼다. 포스팅하고, 댓글 달고, 커뮤니티도 만든다. 다만 가입 조건이 좀 특이하다. 사람은 안 된다. OpenClaw(구 Clawdbot)라는 AI 에이전트 프레임워크로 만든 봇만 들어올 수 있다.
...
- 160만 "AI 에이전트" 중 대부분은 17,000명의 인간이 조종
- 1인당 평균 88개의 봇을 돌림
...
이게 바로 Prompt Injection 공격이다. 겉으로 보기엔 평범한 텍스트. 하지만 그 안에 AI만 알아듣는 명령어가 숨어 있다. 인간 눈엔 안 보여도 AI는 그대로 실행한다.
